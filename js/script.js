...
const products = [
    ...
    },
    {
        ...
        category: 'غرف المعيشة',
        badge: '-25%',
        image: 'https://images.unsplash.com/photo-1565191999001-551c187427bb?auto=format&fit=crop&w=900&q=80',
        rating: 4.6,
        reviews: 89,
        colors: ['خشبي', 'أسود'],
        material: 'خشب وحديد',
...
function openProductDetails(productId) {
    ...
    if (cartSidebar?.classList.contains('active')) closeDialog(cartSidebar, cartOverlay);
    if (wishlistSidebar?.classList.contains('active')) closeDialog(wishlistSidebar, wishlistOverlay);
    ...
    const detailsWishlistBtn = document.querySelector('.details-add-to-wishlist');
    if (detailsWishlistBtn) {
        detailsWishlistBtn.classList.toggle(
            'active',
...
function handleSharedProductFromUrl() {
    const productId = parseInt(new URLSearchParams(window.location.search).get('product'), 10);
    if (!productId) return;
...
function renderProductReviews(productId) {
    ...
    if (reviews.length === 0) {
        container.innerHTML = '<p style="text-align: center; color: var(--text-secondary); padding: 20px;">لا توجد تقييمات بعد. كن أول من يقيّم!</p>';
        return;
    }
...
function renderCompareModal() {
    ...
    if (compareList.length === 0) {
        container.innerHTML = '<p style="text-align: center; padding: 40px; color: var(--text-secondary);">لا توجد منتجات للمقارنة</p>';
        return;
    }
...
function updateCartDisplay() {
    ...
    if (cart.length === 0) {
        cartItems.innerHTML = `
            <div style="text-align: center; padding: 60px 20px; color: var(--text-secondary);">
                ...
                <button data-action="toggle-cart" style="margin-top: 20px; padding: 12px 30px; background: var(--primary); color: white; border: none; border-radius: 25px; cursor: pointer;">تابع التسوق</button>
            </div>
        `;
        return;
    }
    ...
    cart.forEach((item, index) => {
        const productId = findProductIdByName(item.name);
        const clickAttr = productId ? `onclick="openProductDetails(${productId})" style="cursor: pointer;"` : '';
        html += `
            <div class="cart-item">
                <img src="${escapeHTML(item.image || FALLBACK_IMG)}" alt="${escapeHTML(item.name)}" ${clickAttr}>
                <div class="cart-item-info">
                    <div class="cart-item-name" ${clickAttr}>${escapeHTML(item.name)}</div>
...
function addToWishlist(productName, price, button, image = FALLBACK_IMG) {
    ...
    const detailsWishlistBtn = document.querySelector('.details-add-to-wishlist');
    if (detailsWishlistBtn && currentQuickViewProduct?.name === productName) {
        detailsWishlistBtn.classList.toggle(
            'active',
...
async function shareProduct(product) {
    const shareUrl = product.id ? buildProductShareUrl(product.id) : window.location.href;
    const shareData = {
        title: product.name,
    ...
    try {
        ...
        showToast('تعذرت المشاركة على هذا المتصفح.', 'error');
    }
}
...
function updateWishlistCount() {
    ...
    if (mobileBadge) {
        ...
        mobileBadge.style.display = wishlist.length > 0 ? 'flex' : 'none';
    }
}
    ...
    resultsContainer.innerHTML = results.map(p => `
        <div class="search-result-item" onclick="openProductDetails(${p.id}); toggleSearch();">
            ...
            <div>
                ...
                <div style="font-size: 12px; color: var(--text-secondary);">${escapeHTML(p.category)}</div>
            </div>
        </div>
    `).join('');
}
...
function initProductEventDelegation() {
    ...
    // Capture card actions before the image wrapper opens the details modal.
    grid.addEventListener('click', (event) => {
        ...
        if (shareBtn) {
            ...
            const name = card.dataset.name || card.querySelector('.product-name')?.textContent || 'منتج';
            const price = parseInt(card.dataset.price, 10) || parsePriceText(card.querySelector('.current-price')?.textContent);
            const product = products.find(item => item.id === productId) || { id: productId, name, price };
...
// Track scroll position to update mobile bottom nav active state
window.addEventListener('scroll', debounce(() => {
    ...
    navItems.forEach(item => {
        ...
        if (action === 'scroll-home' && currentSection === 'home') {
            item.classList.add('active');
        } else if (action === 'scroll-categories' && (currentSection === 'categories' || currentSection === 'products')) {
            item.classList.add('active');
        } else if (action !== 'toggle-cart' && action !== 'toggle-wishlist' && action !== 'toggle-search') {
